...
export default function SearchClientContainer({
  ...
  const [people, setPeople] = useState<UserProfile[]>(initialPeople)
  const [jobs, setJobs] = useState<LocalJob[]>(initialJobs)
  const [needs, setNeeds] = useState<NeedPost[]>(initialNeeds)
  ...
  // If user is authenticated, refresh live data from RTDB
  useEffect(() => {
    if (!user) return
  ...
  const filteredPosts = posts.filter((p) => {
    ...
    const author = (p.userName || (p as any).authorName || '').toLowerCase()
  ...
  const filteredNeeds = needs.filter((n) => {
    ...
    const author = (n.userName || (n as any).requesterName || '').toLowerCase()
    const area = (n.area || '').toLowerCase()
    return title.includes(q) || desc.includes(q) || author.includes(q) || area.includes(q)
  })
      ...
      <div className="mx-auto max-w-2xl px-4 py-6 md:px-6 space-y-6">
        {isSearching ? (
          ...
          <div className="rounded-3xl border border-dashed border-border bg-card p-10 text-center">
            ...
            <h3 className="mt-3 text-sm font-bold text-slate-900 dark:text-white">No results found for &ldquo;{searchTerm}&rdquo;</h3>
              ...
              <section className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {filteredPeople.map((p) => {
                    ...
                    let distLabel = ''
                    if (userCoords && typeof (p as any).latitude === 'number' && typeof (p as any).longitude === 'number') {
                      const d = getDistanceKm(userCoords.latitude, userCoords.longitude, (p as any).latitude, (p as any).longitude)
                      if (d <= 25) {
                  ...
                  {filteredNeeds.map((n) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-muted-foreground mt-2">
                        <span>Posted by {(n as any).userName || (n as any).requesterName || 'Community Member'}</span>
                        <span>•</span>
                        <span>📍 {n.area}</span>
                      </div>
                    </Link>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="rounded-full bg-amber-500/10 px-2 py-0.5 text-[10px] font-bold text-amber-600 dark:text-amber-400 border border-amber-500/20">
                          {(e as any).eventDate || (e as any).date || 'Upcoming'}
                        </span>
                      </div>
                      <p className="text-xs text-muted-foreground mt-1 line-clamp-2">
                        {e.description}
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-muted-foreground mt-2">
                        ...
                        {((e as any).eventTime || (e as any).time) && (
                          <>
                            <span>•</span>
                            <span>🕒 {(e as any).eventTime || (e as any).time}</span>
                          </>
                        )}
                      </div>
                    </Link>
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-between">
      ...
      <main className="flex-1">
        ...
        <div className="mx-auto max-w-2xl px-4 pb-12 md:px-6">
          <OpenInCircularBanner path="/search" title="Search Circular" />
        </div>
      </main>
      <CircularFooter />
    </div>
